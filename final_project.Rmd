...
I am using independt sample t-test to see if coffees with shipped cream are more heavy in calories thank the ones without
Nullhypothesis is that there is no difference, alternative hypothisis is that the whipped creamed have more calories

I have a big enough data, but I am checking the normality anyway
...
It is a bit skewed, but I have a lot of data, so I am accepting it and moving to the model
I am calculating the model

```{r}
whip_t_test <- t.test(calories ~ whip, data = starbucks)
whip_t_test
```

It seems like we have a significant difference in the calories of whipped and not whipped coffees. t(502.9) = -24.87, p < 0.001
...
```{r}
starbucks_anova <- oneway.test(calories ~ size, data = starbucks, var.equal = FALSE)
starbucks_anova
```

I found a significant model (F(147.57,4)= 105.78, p < 0.001)

To find out which groups differ from each other I have run a pairwise t-test with Bonferroni correction

```{r}
pairwise.t.test('calories', 'size', data = starbucks, p.adj = "bonf")

TukeyHSD(starbucks_anova, "size")
```
...
Lastly I wanted to see the relation between size, whipped cream and calories of coffees
...
ggplot(starbucks_to_plot, aes(x = fct_reorder(size, calories), y = calories, fill = whip)) +
  geom_col()  +
  theme_light () +
  scale_fill_viridis_d() +
  labs (title = "Starbucks coffees by size, calories and whipped cream", x = "Size", y = "Calories") +
